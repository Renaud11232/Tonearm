...
class StorageService:
    ...
    def get_dj_roles(self) -> List[discord.Role]:
    ...
    def __set_dj_roles(self, roles: List[discord.Role]):
            int_roles = list(map(lambda role: role.id, roles))
            self.__set("dj.roles", int_roles)

    def set_dj_roles(self, dj_roles: List[discord.Role]):
        self.__set_dj_roles(dj_roles)

    def get_dj_members(self) -> List[discord.Member]:
    ...
    def __set_dj_members(self, members: List[discord.Member]):
        int_members = list(map(lambda member: member.id, members))
        self.__set("dj.members", int_members)

    def set_dj_members(self, members: List[discord.Member]):
        self.__set_dj_members(members)
    ...
    def __set_channel(self, channel: discord.TextChannel | None):
    ...
    def get_locale(self) -> discord.Locale:
        str_locale = self.__get("locale", default=str(discord.Locale.american_english))
        try:
            locale = discord.Locale(str_locale)
        except ValueError:
            locale = discord.Locale.american_english
        if str_locale != str(locale):
            self.__set_locale(locale)
        return locale

    def __set_locale(self, locale: discord.Locale):
        str_locale = str(locale)
        self.__set("locale", str_locale)

    def set_locale(self, locale: discord.Locale):
        self.__set_locale(locale)